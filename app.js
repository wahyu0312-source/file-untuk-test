/* =========================================================
 * app.js — Tokyo Seimitsu ERP Frontend (stable + compat)
 * ========================================================= */
...
async function rawFetch(url, opt){
  ...
  return {ok: res.ok, json: j, text: txt};
}

async function apiPost(action, body){
  const payload = { action, ...body };
  if(API_KEY) payload.apiKey = API_KEY;
  const {json, text} = await rawFetch(API_BASE, {
    method:'POST',
    headers:{'Content-Type':'text/plain;charset=utf-8'},
    body:JSON.stringify(payload),
    cache:'no-store'
  });
  if(!json){ console.error('API RAW (POST '+action+'):\n', text); throw new Error('Invalid response (cek deploy/izin Apps Script).'); }
  if(json.ok === false){ throw new Error(json.error || 'API error'); }
  if(json.ok !== true && json.data === undefined){ console.error('API RAW (POST '+action+'):\n', text); throw new Error('Invalid response (cek deploy/izin Apps Script).'); }
  return json.data ?? json; // toleransi
}
...
async function apiGet(params, {swrKey=null, revalidate=true}={}){
  ...
  if (cached && revalidate){
    rawFetch(url,{cache:'no-store'}).then(({json})=>{
      if(json && (json.ok===true || json.data!==undefined)){
        SWR.set(key, json.data ?? json);
  ...
  const {json, text} = await rawFetch(url,{cache:'no-store'});
  if(!json){ console.error('API RAW (GET):\n', text); if(cached) return cached; throw new Error('Invalid response (cek deploy/izin Apps Script).'); }
  if(json.ok === false){ throw new Error(json.error || 'API error'); }
...
/* ====== API COMPAT (coba beberapa nama action) ====== */
async function tryGetOne(actions, params={}, {swrKeyPrefix=''}={}){
  let lastErr = null;
  for(const action of actions){
    try{
      const data = await apiGet({action, ...params}, {swrKey:`${swrKeyPrefix}${action}`, revalidate:false});
      return {action, data};
    }catch(err){
      lastErr = err;
      // kalau error Unknown action → lanjut coba berikutnya
      if(!String(err?.message||'').toLowerCase().includes('unknown action')) break; // tetap break bila backend tidak pakai pola ok:false
    }
  }
  throw lastErr || new Error('No compatible action found');
}
...
function showApiError(action, err){
  ...
  bar.innerHTML = `<b>APIエラー</b> <code>${action}</code> — ${msg}`;
}
...
async function loadDashboard(){
  ...
  try{
    // coba beberapa nama action:
    const {action, data:d} = await tryGetOne(
      ['dashboard','getDashboard','dash','home','summary','stats','stat'],
      {},
      {swrKeyPrefix:'dash:'}
    );

    // ringkasan
    $('#statFinished') && ($('#statFinished').textContent = d?.finished ?? d?.完成在庫 ?? d?.summary?.finished ?? '-');
    $('#statReady')    && ($('#statReady').textContent    = d?.ready    ?? d?.出荷準備 ?? d?.summary?.ready    ?? '-');
    $('#statShipped')  && ($('#statShipped').textContent  = d?.shipped  ?? d?.出荷済   ?? d?.summary?.shipped  ?? '-');

    // list hari ini (fallback ke d.today / d.orders / d.当日)
    const list = d?.today || d?.orders || d?.当日 || [];
    if(body){
      clearSkeleton(body);
      body.innerHTML = (list||[]).map((x,i)=>`
        <tr><td>${i+1}</td><td>${x.po||x.PO||x.注番||''}</td><td>${x.customer||x.得意先||''}</td><td>${fmtD(x.due||x.納期)}</td></tr>
...
function destroyChart(id){
  ...
  if(el){ const inst = Chart.getChart(el) || Chart.getChart(id); if(inst) try{ inst.destroy(); }catch(_){} }
}
...
async function renderCharts(){
  ...
  try{
    // coba berbagai action charts / statistics
    const {data:d} = await tryGetOne(
      ['charts','getCharts','statistics','stats','getStatistics'],
      {year},
      {swrKeyPrefix:'charts:'}
    );
    ...
    drawBar('chMonthly', ms(d?.perMonth || d?.month || d?.月別 || []));
    drawPie('chCustomer', d?.perCust || d?.customer || d?.得意先内訳 || {});
    drawPie('chStock', d?.stockBuckets || d?.stock || d?.在庫区分 || {});
    drawBar('chWipProc', objToBar(d?.wipByProcess || d?.wip || d?.工程WIP || {}));
    drawBar('chSales', ms(d?.salesPerMonth || d?.sales || d?.売上 || []));
    drawBar('chPlan',  ms(d?.planPerMonth  || d?.plan  || d?.計画 || []));
...
async function previewInvoiceUI(){
  ...
  try{
    const {data:d} = await tryGetOne(
      ['previewInvoice','invoicePreview','getInvoicePreview','listInvoiceItems','invPreview'],
      { customer:info.customer, from:info.from, to:info.to },
      {swrKeyPrefix:'inv:'}
    );
    ...
    INV_PREVIEW.lines = d?.lines || d?.items || [];
