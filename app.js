...
let CACHE = { orders:[], sales:[], todayShip:[], locSnap:{}, stock:{} };
...
const fmtDate = d => {
  if(!d) return '';
  const x = new Date(d); if(Number.isNaN(+x)) return '';
...
const parseCSV = async (file) => {
  ...
  for(let i=1;i<lines.length;i++){
    ...
    cols.push(cur);
    const o={}; head.forEach((h,idx)=>o[h]=cols[idx]??''); rows.push(o);
  }
  return rows;
};
const badge = (status)=>{
  const map = {'生産開始':'info','検査保留':'hold','検査済':'ok','出荷準備':'warn','出荷済':'ship','不良品（要リペア）':'ng'};
  const cls = map[status] || 'muted';
  return `<span class="badge ${cls}">${status||'-'}</span>`;
};
const procLabelClass = (p)=>{
  const map = {'レーザ加工':'laser','曲げ加工':'bend','外枠組立':'frame','シャッター組立':'sh-assy','シャッター溶接':'sh-weld','コーキング':'caulk','外枠塗装':'paint','組立（組立中）':'asm-in','組立（組立済）':'asm-ok','外注':'out','検査工程':'inspect'};
  return map[p]||'';
};
...
async function apiGet(params){
  ...
  try{
    ...
    let j; try{ j = JSON.parse(txt); } catch{ throw new Error('API parse error: '+txt.slice(0,80)); }
...
function afterLogin(){
  ...
  show($('#btnToDash'), true); show($('#btnToSales'), true); show($('#btnToPlan'), true);
  show($('#btnToShip'), true); show($('#btnToInvoice'), true); show($('#btnToCharts'), true);
  ...
  show($('#btnChangePass'), true); show($('#btnLogout'), true);
  show($('#authView'), false);
  navigate('pageDash'); refreshDashboard(); loadSalesList();
}
...
async function refreshDashboard(){
  const [orders, today, loc, stock] = await Promise.all([
    apiGet({action:'listOrders'}), apiGet({action:'todayShip'}), apiGet({action:'locSnapshot'}), apiGet({action:'stock'})
  ]);
  CACHE.orders = orders; CACHE.todayShip = today; CACHE.locSnap = loc; CACHE.stock = stock;
  ...
  const chips = Object.entries(loc).map(([p,n])=>`
    <div class="grid-chip proc ${procLabelClass(p)}">
      <div class="s">${p}</div><div class="h">${n}</div>
    </div>`).join('');
...
$('#tbOrders').addEventListener('click', async (e)=>{
  ...
  if(act==='history'){
    ...
    $('#histBody').innerHTML = logs.map(l=>(
      ...
      </div>`)).join('') || '<div class="muted s">履歴なし</div>';
    $('#dlgHistory').showModal();
  }
  if(act==='scan'){ openScan(po); }
  if(act==='del'){
    if(!confirm(`削除しますか？ ${po}`)) return;
    try{ await apiPost({action:'deleteOrder', po_id:po, user:CURRENT_USER}); await refreshDashboard(); }
...
function fillSalesForm(o){
  $('#so_id').value=o.so_id||''; $('#so_date').value=fmtDate(o['受注日']);
  $('#so_cust').value=o['得意先']||''; $('#so_item').value=o['品名']||'';
  $('#so_part').value=o['品番']||''; $('#so_drw').value=o['図番']||'';
  $('#so_sei').value=o['製番号']||''; $('#so_qty').value=o['数量']||'';
  $('#so_req').value=fmtDate(o['希望納期']); $('#so_note').value=o['備考']||'';
}
function readSalesForm(){
  return {'受注日':$('#so_date').value,'得意先':$('#so_cust').value.trim(),'品名':$('#so_item').value.trim(),'品番':$('#so_part').value.trim(),'図番':$('#so_drw').value.trim(),'製番号':$('#so_sei').value.trim(),'数量':Number($('#so_qty').value||0),'希望納期':$('#so_req').value||'','備考':$('#so_note').value.trim()};
}
async function loadSalesList(){
  const q = $('#salesQ').value||'';
  const rows = await apiGet({action:'listSales', q}); CACHE.sales = rows;
  $('#tbSales').innerHTML = rows.map(r=>`
    <tr data-id="${r.so_id}">
      <td>${r.so_id}</td><td>${fmtDate(r['受注日'])}</td><td>${r['得意先']||''}</td>
      <td>${r['品名']||''}</td><td>${(r['品番']||'')}/${(r['図番']||'')}</td>
      <td>${r['数量']||0}</td><td>${fmtDate(r['希望納期'])}</td><td>${r.status||''}</td>
      <td>${r.linked_po_id||''}</td><td>${fmtDate(r.updated_at)}</td>
    </tr>`).join('');
}
$('#tbSales').addEventListener('click', (e)=>{
  const tr = e.target.closest('tr[data-id]'); if(!tr) return;
  const so = CACHE.sales.find(x=>x.so_id===tr.dataset.id); if(so) fillSalesForm(so);
});
$('#btnSalesSave').onclick = async ()=>{
  try{
    ...
    await loadSalesList(); alert('保存しました');
  }catch(e){ alert(e.message); }
};
$('#btnSalesDelete').onclick = async ()=>{
  const id = $('#so_id').value; if(!id) return alert('SOが空です');
  if(!confirm('削除しますか？')) return;
  try{ await apiPost({action:'deleteSalesOrder', so_id:id, user:CURRENT_USER}); fillSalesForm({}); await loadSalesList(); }
  catch(e){ alert(e.message); }
};
$('#btnPromote').onclick = async ()=>{
  const id = $('#so_id').value; if(!id) return alert('SOを選択してください');
  try{ const r = await apiPost({action:'promoteSalesToPlan', so_id:id, user:CURRENT_USER}); alert('計画へ変換: '+r.po_id); navigate('pagePlan'); refreshDashboard(); }
  catch(e){ alert(e.message); }
};
...
$('#fileSalesCSV').addEventListener('change', async (e)=>{
  ...
  const rows = await parseCSV(f); if(!rows.length) return alert('CSV kosong');
  if(!confirm(`Import ${rows.length} baris ke Sales?`)) return;
  for(const r of rows){
    try{
      await apiPost({action:'createSalesOrder', payload:{
        '受注日':r['受注日']||r['so_date']||'','得意先':r['得意先']||r['customer']||'','品名':r['品名']||r['item']||'','品番':r['品番']||r['part']||'','図番':r['図番']||r['drw']||'','製番号':r['製番号']||r['sei']||'','数量':r['数量']||r['qty']||0,'希望納期':r['希望納期']||r['req']||'','備考':r['備考']||r['note']||''
      }, user:CURRENT_USER});
    }catch(err){ console.warn(err.message); }
  }
  await loadSalesList(); alert('Import selesai');
});
...
$('#btnCreateOrder').onclick = async ()=>{
  try{ const r = await apiPost({action:'createOrder', payload:planPayloadFromForm(), user:CURRENT_USER}); $('#c_po').value = r.po_id; await refreshDashboard(); alert('現品票を発行: '+r.po_id); }
  catch(e){ alert(e.message); }
};
$('#btnPlanDelete').onclick = async ()=>{
  const po = $('#c_po').value; if(!po) return alert('注番が空です');
  if(!confirm('削除しますか？')) return;
  try{ await apiPost({action:'deleteOrder', po_id:po, user:CURRENT_USER}); $('#c_po').value=''; await refreshDashboard(); }
  catch(e){ alert(e.message); }
};
  ...
  for(const r of rows){
    try{
      await apiPost({action:'createOrder', payload:{
        '得意先':r['得意先']||'','得意先品番':r['得意先品番']||'','製番号':r['製番号']||'','品名':r['品名']||'','品番':r['品番']||'','図番':r['図番']||'','通知書番号':r['通知書番号']||'','管理No':r['管理No']||''
      }, user:CURRENT_USER});
    }catch(err){ console.warn(err.message); }
  }
  await refreshDashboard(); alert('Import selesai');
});
...
$('#btnSchedule').onclick = async ()=>{
  const p = readShipForm(); if(!p.po || !p.date) return alert('注番と日付');
  try{ const r = await apiPost({action:'scheduleShipment', po_id:p.po, dateIso:p.date, qty:p.qty, user:CURRENT_USER}); $('#s_shipid').value=r.ship_id; await refreshDashboard(); alert('出荷予定を登録'); }
  catch(e){ alert(e.message); }
};
$('#btnShipEdit').onclick = async ()=>{
  const p = readShipForm(); if(!p.shipid) return alert('出荷IDが空です');
  try{ await apiPost({action:'updateShipment', ship_id:p.shipid, updates:{po_id:p.po, scheduled_date:p.date, qty:p.qty}, user:CURRENT_USER}); await refreshDashboard(); alert('更新しました'); }
  catch(e){ alert(e.message); }
};
$('#btnShipDelete').onclick = async ()=>{
  const id = $('#s_shipid').value; if(!id) return alert('出荷IDが空です');
  if(!confirm('削除しますか？')) return;
  try{ await apiPost({action:'deleteShipment', ship_id:id, user:CURRENT_USER}); $('#s_shipid').value=''; await refreshDashboard(); }
  catch(e){ alert(e.message); }
};
$('#btnShipByPO').onclick = async ()=>{
  const p = $('#s_po').value.trim(); if(!p) return alert('注番を入力');
  try{ const d = await apiGet({action:'shipByPo', po_id:p}); $('#dlgShip .body').innerHTML = buildShipDoc(d.order,d.shipment); $('#dlgShip').showModal(); }
  catch(e){ alert(e.message); }
};
$('#btnShipByID').onclick = async ()=>{
  const id = $('#s_shipid').value.trim(); if(!id) return alert('出荷IDを入力');
  try{ const d = await apiGet({action:'shipById', ship_id:id}); $('#dlgShip .body').innerHTML = buildShipDoc(d.order,d.shipment); $('#dlgShip').showModal(); }
  catch(e){ alert(e.message); }
};
...
$('#fileShipCSV').addEventListener('change', async (e)=>{
  ...
  for(const r of rows){
    try{ await apiPost({action:'scheduleShipment', po_id:r['po_id']||r['注番']||'', dateIso:r['scheduled_date']||r['日付']||r['date']||'', qty:r['qty']||r['数量']||0, user:CURRENT_USER}); }
    catch(err){ console.warn(err.message); }
  }
  await refreshDashboard(); alert('Import selesai');
});
...
function renderInvoiceLines(rows){
  const tbody = $('#invLines'); let n=0, sub=0;
  tbody.innerHTML = rows.map(r=>{
    const price = Number(r['単価']||0), qty = Number(r['数量']||0), amt = (price||0)*qty; sub+=amt;
    return `<tr><td>${++n}</td><td>${r['品名']||''}</td><td>${r['品番']||''}</td><td>${r['図番']||''}</td>
      <td contenteditable="true" data-col="数量">${qty}</td><td contenteditable="true" data-col="単価">${price}</td>
      <td>${amt}</td><td>${r['po_id']||''}</td><td>${r['ship_id']||''}</td></tr>`;
  }).join('');
  const tax = Math.round(sub*0.10);
  $('#invSub').textContent=sub; $('#invTax').textContent=tax; $('#invTotal').textContent=sub+tax;
}
$('#btnInvCSV').onclick = ()=>{
  const rows=[]; $('#tblInv tbody tr').forEach((tr,idx)=>{const t=tr.querySelectorAll('td'); rows.push({'#':idx+1,'品名':t[1].textContent.trim(),'品番':t[2].textContent.trim(),'図番':t[3].textContent.trim(),'数量':t[4].textContent.trim(),'単価':t[5].textContent.trim(),'金額':t[6].textContent.trim(),'注番':t[7].textContent.trim(),'出荷ID':t[8].textContent.trim()});});
...
async function startScan(){
  ...
  if(window.ZXing && ZXing.BrowserMultiFormatReader){
    ...
    try{ await _codeReader.decodeFromVideoDevice(undefined,$('#scanVideo'),(result,err)=>{ if(result) handleScanText(result.getText()); }); }
    catch(e){ alert('Camera gagal: '+e.message); }
  }else alert('ZXing belum siap');
}
function stopScan(){ try{ _codeReader && _codeReader.reset(); }catch{} _codeReader=null; }
async function decodeQRFromFile(file){
  const img=new Image(), url=URL.createObjectURL(file);
  await new Promise((res,rej)=>{ img.onload=()=>res(); img.onerror=()=>rej(new Error('Image load error')); img.src=url; });
  const cvs=$('#scanCanvas'), ctx=cvs.getContext('2d'); cvs.width=img.naturalWidth; cvs.height=img.naturalHeight; ctx.drawImage(img,0,0);
  const d=ctx.getImageData(0,0,cvs.width,cvs.height); const qr=jsQR(d.data,d.width,d.height); URL.revokeObjectURL(url);
  if(!qr) throw new Error('QR tidak terbaca'); return qr.data;
}
async function handleScanText(text){
  $('#scanResult').textContent='Read: '+text;
  const lines=String(text).split(/\r?\n/).map(s=>s.trim()).filter(Boolean);
  const upd={}; let note='', proc='', status='';
  lines.forEach(l=>{ const [k,...rest]=l.split(':'); const v=rest.join(':').trim();
    if(/^ST$/i.test(k)) proc=v; else if(/^STATUS$/i.test(k)) status=v; else if(/^NOTE$/i.test(k)) note=v; else if(/^PO$/i.test(k)) _currentScanPO=v; });
  if(!proc && !status && lines.length===1 && lines[0].startsWith('ST:')) proc = lines[0].slice(3).trim();
  if(proc) upd.current_process=proc; if(status) upd.status=status; if(note) upd.note=note;
  if(!_currentScanPO) return alert('PO tidak diketahui. QR harus mengandung PO:xxxx atau buka dari daftar.');
  try{ await apiPost({action:'updateOrder', po_id:_currentScanPO, updates:upd, user:CURRENT_USER}); await refreshDashboard(); }catch(e){ alert('Update gagal: '+e.message); }
}

// ===== STATION QR =====
$('#btnShowStationQR').onclick = ()=>{
  const wrap=$('#qrWrap'); wrap.innerHTML='';
  const procs=['レーザ加工','曲げ加工','外枠組立','シャッター組立','シャッター溶接','コーキング','外枠塗装','組立（組立中）','組立（組立済）','外注','検査工程','生産開始','検査保留','検査済','出荷準備','出荷済','不良品（要リペア）'];
  procs.forEach(p=>{
    const box=document.createElement('div'); box.style.padding='8px'; box.style.textAlign='center'; box.style.border='1px solid #e5e7eb'; box.style.borderRadius='10px';
    const q=document.createElement('div'); q.style.margin='6px auto'; new QRCode(q,{text:`ST:${p}`,width:120,height:120});
    box.innerHTML=`<div class="s muted">${p}</div>`; box.prepend(q); wrap.appendChild(box);
  });
...
$('#btnNewUser').onclick = async ()=>{
  const p={ username:$('#nuUser').value.trim(), password:$('#nuPass').value, full_name:$('#nuName').value.trim(), department:$('#nuDept').value, role:$('#nuRole').value };
  try{ await apiPost({action:'createUser', user:CURRENT_USER, payload:p}); alert('ユーザー追加しました'); }catch(e){ alert(e.message); }
};
$('#btnChangePass').onclick = async ()=>{
  const oldp=prompt('旧パスワード'); if(oldp==null) return;
  const newp=prompt('新しいパスワード'); if(newp==null) return;
  try{ await apiPost({action:'changePassword', user:CURRENT_USER, oldPass:oldp, newPass:newp}); alert('変更しました'); }catch(e){ alert(e.message); }
};
$('#btnLogout').onclick = ()=>{
  CURRENT_USER=null; show($('#authView'),true); ['pageDash','pageSales','pagePlan','pageShip','pageInvoice'].forEach(id=>show($('#'+id),false));
