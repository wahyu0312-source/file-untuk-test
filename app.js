...
async function apiGet(params){
  ...
  try{
    ...
    try{ j=JSON.parse(txt); }catch{ throw new Error('API parse error: '+txt.slice(0,100)); }
...
async function doLogin(){
  ...
  try{ const me=await apiPost({action:'login',username:u,password:p}); CURRENT_USER=me; afterLogin(); }
...
function afterLogin(){
  ...
  show($('#authView'),false); navigate('pageDash'); refreshDashboard(); loadSalesList();
}
...
async function refreshDashboard(){
  const [orders,today,loc,stock]=await Promise.all([apiGet({action:'listOrders'}),apiGet({action:'todayShip'}),apiGet({action:'locSnapshot'}),apiGet({action:'stock'})]);
...
$('#btnSalesSave').onclick=async()=>{ try{ const payload=readSalesForm(); if(!$('#so_id').value){ const r=await apiPost({action:'createSalesOrder',payload,user:CURRENT_USER}); $('#so_id').value=r.so_id; } else { await apiPost({action:'updateSalesOrder',so_id:$('#so_id').value,updates:payload,user:CURRENT_USER}); } await loadSalesList(); alert('保存しました'); }catch(e){ alert(e.message); } };
$('#btnSalesDelete').onclick=async()=>{ const id=$('#so_id').value; if(!id) return alert('SOが空です'); if(!confirm('削除しますか？')) return; try{ await apiPost({action:'deleteSalesOrder',so_id:id,user:CURRENT_USER}); fillSalesForm({}); await loadSalesList(); }catch(e){ alert(e.message); } };
...
/* 11) INVOICE (client-side) */
let INV_CACHE=[]; $('#btnInvPreview').onclick=async()=>{ const cust=$('#inv_customer').value.trim(),from=$('#inv_from').value,to=$('#inv_to').value; if(!cust||!from||!to) return alert('得意先と期間を入力'); const rows=await apiGet({action:'finishedStockList'}); INV_CACHE=rows.filter(r=>(r['得意先']||'').includes(cust)); renderInvoiceLines(INV_CACHE); };
